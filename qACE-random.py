...
def main():
    ...
    assert aw_np.shape[0] == bw_np.shape[0]
    assert energy_selector.shape[0] == bw_np.shape[0]
    assert force_selector.shape[0] == bw_np.shape[0]

    # --- Feature selection: randomly select 50% of columns ---
    # Commented out to use all columns
    # start_fs = time.time()
    # total_cols = aw_np.shape[1]
    # np.random.seed(1)
    # selected_indices = np.random.choice(total_cols, total_cols // 10, replace=False)
    # reduced_aw_np = aw_np[:, selected_indices]
    # feature_selection_time = time.time() - start_fs
    # print(f"Random feature selection (10%) took {feature_selection_time:.3f} sec.")

    # Use full feature set
    full_aw_np = aw_np

    # --- Train-test split ---
    # Test set is the first 1/8 of rows
    hlfpnt = full_aw_np.shape[0] // 8
    dA_test = full_aw_np[:hlfpnt, :]
    d_b_test = bw_np[:hlfpnt]

    # Training set is the rest of the rows
    dA_train_full = full_aw_np[hlfpnt:, :]
    d_b_train_full = bw_np[hlfpnt:]
    
    energy_selector_test = energy_selector[:hlfpnt]
    force_selector_test = force_selector[:hlfpnt]
    
    # Use only half of the training rows (first half)
    train_half = dA_train_full.shape[0] // 10
    dA_train = dA_train_full[:train_half, :]
    d_b_train = d_b_train_full[:train_half]
    
    energy_selector_train = energy_selector[hlfpnt:hlfpnt+train_half]
    force_selector_train = force_selector[hlfpnt:hlfpnt+train_half]
